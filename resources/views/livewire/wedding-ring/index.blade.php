				              ...
				              <button class="btn btn-outline {{$search_conditions['shape'][$key]['clicked']? 'btn-active':'' }}"  type="button" wire:click="toggleValue('shape', '{{$key}}' )">
				                    {{__('weddingRing.' . $key)}}
						             ...
						             @endif
				                   	...
				                   	@if($key == 'wave')						             
						             	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 438.406 344.308"><rect width="100%" height="100%" fill="#FFF"/><g class="currentLayer"><path fill="#242424" stroke="#222" stroke-linejoin="round" d="M539.817 233.352l-59.571-63.972-72.881 66.126-58.83-64.085-72.938 66.958-60.337-65.726-71.158 65.325-59.592-64.914-71.158 65.324-88.643-96.56 35.579-32.661 58.847 64.102 72.047-66.14 59.592 64.913 72.048-66.14 59.592 64.913 72.048-66.14 60.337 65.725 72.937-66.958 89.388 97.371c-13.114 9.372-22.81 25.486-37.307 32.539h0z" class="selected" color="#000"/></g><defs><filter id="f205" height="1" width="1" y="0" x="0" color-interpolation-filters="sRGB"><feGaussianBlur result="result6" stdDeviation=".4"/><feComposite in2="result6" operator="in" in="SourceGraphic" result="result7"/><feComposite operator="in" in2="result7" result="result8" in="result7"/></filter></defs></svg>
						             @endif
