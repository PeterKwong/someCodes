<div  x-data="{ 'showModal': false }">
    ...
    @if($showModal)
        ...
        <!--Modal-->
        <div class="flex overflow-hidden z-50" style="background-color: rgba(0,0,0,0.5)" x-show="showModal" :class="{ ' fixed inset-0 w-full h-full': showModal }">
            <div class="absolute overflow-y-auto md:fixed top-10 md:top-auto md:bottom-0 bg-white md:flex flex-col items-center justify-center w-full h-5/6 p-2">
                <div class="flex w-full justify-end p-7">
                    <button class="text-2xl text-brown focus:outline-none" @click="showModal = false">
                        &times;
                    </button>
                </div>
            </div>
