<form action="#" :class="{'hidden lg:flex' : !applyFilter, 'flex lg:hidden fixed overflow-y-scroll h-5/6 w-full top-10 left-0 z-50 bg-white pt-5 md:pt-0 md:pb-0 px-4 md:px-0 border border-2' : applyFilter,}" class="flex-col space-y-5 font-lato" x-on:click.away="applyFilter = false; mobileUpdateInputs()">
    ...
    <a class="flex md:hidden items-center justify-center w-full mt-7 py-2 bg-brown hover:bg-white text-white hover:text-brown border border-brown transition ease-in-out duration-500">
        <span class="text-white hover:text-brown font-bold font-lato tracking-widest uppercase"  @click="applyFilter = false; mobileUpdateInputs()">{{trans('weddingRing.Apply')}}</span>
    </a>
