<template>
	...
	<h1 class="title is-3" v-if="option">Latest ID:{{option.id}}</h1>
		...
		<div class="field" >
					...
					<div class="control has-icon-left">
						<div class="control">
							<label class="label">Gemstone</label>
								<select v-model="form.gemstone" class="select">
					...
					<div class="control has-icon-left">
						<div class="control">
							<label class="label">Metal</label>
								<select v-model="form.metal" class="select">
					...
					<div class="control has-icon-left">
						<div class="control">
							<label class="label">Setting</label>
								<select v-model="form.setting" class="select">
			...
			<div class="columns" @submit.prevent="fetchData">
	...
	export default {
		...
		beforeMount(){
			if (this.$route.meta.mode === 'edit') {
				this.title = 'Edit'
				this.initialize = '/api/jewellery/' + this.$route.params.id + '/edit'
				this.storeURL = `/api/jewellery/${this.$route.params.id}?_method=PUT`
				this.method = 'put'
			}
			this.fetchData()
		},
