            ...
            <div class="col-span-12"  v-if="item.pairItems.length >0">
                <div @click="shiftIndex(firstIndex)">
                    <div class="flex justify-between items-center text-white p-1" :class="item.pairItems.filter((data)=>{return data.available != 1}).length?'bg-red-300': 'bg-blue-300' " >
                        <div class="px-4">
                           <p class="text-xl font-light"> <strong>{{item.pairItems[0].type |transJs() }} </strong><strong v-if="item.pairItems[1]"> + {{ item.pairItems[1].type |transJs() }}</strong></p>
                        </div>

                        <div class="sm:px-4">
                            <a @click="directTo(item.pairItems[0].id)" :href="item.pairItems[0].url + item.pairItems[0].id ">
                                <button class="btn" :class="item.pairItems.filter((data)=>{return data.available != 1}).length?'bg-red-500': 'btn-primary' " >{{ 'Back to'|transJs() }}{{item.pairItems[0].type |transJs()}}</button>
                            </a>
                            <button class="btn" :class="item.pairItems.filter((data)=>{return data.available != 1}).length?'bg-red-500': 'btn-primary' "  @click="removeItem(firstIndex)">{{ 'Remove' |transJs() }}  <i class="fa fa-times-circle"></i></button>
                        </div>
                                    ...
                                    <div class="col-span-8 text-center">
                                        ...
                                        <div v-if="pairItem.type == 'engagementRings' ">    
                                            <div class="flex justify-center">
                                              ...
                                              <input class="input is-small" id="inputGroupSelect01" type="text" name="" v-model="pairItem.engrave" :placeholder="'Engrave' |transJs()">
                                        </div>                                        
                                    </div>                                    
                                </div>
                            </div>    
                            ...
                            <div class="col-span-3">
                                <div class="grid grid-cols-12">
                                    <div class="col-span-6">
                                         <a class="text-blue-600" @click="directTo()" :href="pairItem.url" ><u>{{'change'  |transJs() }}</u></a>
        ...
        <div class="grid grid-cols-12 p-1">
            <div class="col-span-12 sm:col-span-8">
                <div class="grid grid-cols-12">
                    <div class="col-span-12 p-2">
                        <p>{{ 'Precautions'|transJs() }}：</p>                    
                        <small>
                            <p>{{ 'All amounts of the company are subject to Hong Kong dollar settlement' |transJs() }}</p>                    
                            <p>{{ 'The customer is required to pay the full amount and withdraw the goods within two months after the order is placed, otherwise the company reserves the right to terminate the transaction.' |transJs() }}</p>                    
                            <p>{{ 'In order to protect the interests of both parties, unless the diamond does not match the GIA report, the order will not be returned after confirmation.' |transJs() }}</p>                    
                        </small>
                        <br>
                        <div class="rounded border text-center p-10">
                            <p v-if="maxDeliveryDate">{{ 'Today Order, Diamond Gets Free shipment' |transJs() }}
                                <strong>{{ 'On' |transJs() }} <a class="text-blue-600"> {{ extraWorkingDates( maxDeliveryDate ,'months') |transJs() }} {{ extraWorkingDates( maxDeliveryDate ) }} {{ 'day' |transJs() }},  {{ extraWorkingDates( maxDeliveryDate ,'dates') |transJs() }}</a>
                                </strong> 
            ...
            <div class="col-span-12 sm:col-span-4">
                    ...
                    <div class="col-span-12">
                        <div class="flex">
                          ...
                          <div class="input-group-prepend">
                            <button class="btn btn-primary is-small" @click="checkCouponCodeValid" type="button" id="button-addon1">{{'Apply Coupon'|transJs() }}</button>
                          </div>
                          <input type="text" class="input" :placeholder="'Enter Coupon Code' |transJs() " v-model="mutualVar.cookiesInfo.coupon_code" @focus="$event.target.select()" aria-label="Example text with button addon" aria-describedby="button-addon1">
                                                        
                            <p v-if="couponValid == 0 " style="color:red;">
                                <small>{{'not valid'|transJs() }}</small>
                            </p>
                        </div>
                ...
                <div class="grid grid-cols-12">
                    ...
                    <div class="col-span-6">
                        <div v-if="couponValid || calculatedDiscountRate != 1">
                            <p>{{'Total' |transJs() }} </p>
                            <p class="text-red-500">{{'Disounted Total' |transJs() }}</p>
                        </div>
                        <div v-else="!couponValid ">
                            <p>{{'Total' |transJs() }} </p>
                        </div>
                        <p class="text-lg text-gray-600">{{'Deposit (20%)' |transJs() }}</p>
                        <div class="">
                            <p class="text-lg text-blue-600">{{'Balance (80%)'|transJs() }}
                            </p>
                            <select class="border-2 border-gray-600" v-model="mutualVar.cookiesInfo.checkout.balancePaymentMethod">
                                <option v-for="paymentOption in paymentOptions" :value="paymentOption.name"> {{paymentOption.name |transJs() }}</option>
                            </select>  
                ...
                <div class="flex justify-between ">
                    ...
                    <div class="">
                        <p style="color:red;">* {{'you only need to pay deposit, balance will pay after 100% satisfied'|transJs() }}</p>
                    </div>

                    <div  v-if=" windowHref.includes('shopping-cart')"  @click="sendCookies()">
                        <a :href="'/' + locale + '/shop-bag-bill' ">
                        <button class="btn btn-primary" :class="{'opacity-25' : !checkoutClickable}" :disabled="!checkoutClickable"> <i class="fas fa-shopping-cart"></i>{{'checkout' |transJs() }}</button> 
                        </a>
                    </div>                              
