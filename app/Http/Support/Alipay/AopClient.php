...
class AopClient {
	...
	//construct 
	public function __construct($gatewayUrl,$appId,$rsaPrivateKey,$format,$postCharset,$alipayrsaPublicKey,$signType)
	{	
	...
	protected function curl($url, $postFields = null) {
		...
		// $response = $this->guzzleGet($url, substr($postBodyString, 0, -1));
		// $curl = new LoginController();
		// $reponse = $curl->curl_exec_from_controller($ch);
	...
	public function guzzleGet($url,$data){

	      $client = new Client();

	      $request = new Req('post', $url, ['content-type: application/x-www-form-urlencoded;charset=' . $this->postCharset], $data);
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			// 执行加密
			$enCryptContent = aplipayEncrypt($apiParams['biz_content'], $this->encryptKey);
