...
Class Request{
     ...
     * 提交订单信息
     */
    public function submitOrderInfo($paymentData,$serviceType){
        // $this->requestData = $paymentData;
        // dd(123);
        $this->reqHandler->setReqParams($paymentData,array('method'));
        $this->reqHandler->setParameter('service',$serviceType);//接口类型
		...
		$this->reqHandler->setParameter('body',$paymentData['body']);
		
		$this->reqHandler->setParameter('mch_create_ip','127.0.0.1');
	
		$this->reqHandler->setParameter('out_trade_no',mt_rand(time(),time()+rand()));
	
		$this->reqHandler->setParameter('op_device_id','东风一号');
        $this->reqHandler->setParameter('limit_credit_pay','1');
        $this->reqHandler->setParameter('payment_inst','ALIPAYCN');
        ...
        if($this->pay->call()){
            // dd($this->pay->getResContent());
            $this->resHandler->setContent($this->pay->getResContent());
            // dd($this->resHandler->parameters);
            $this->resHandler->setKey($this->reqHandler->getKey());
            if($this->resHandler->isTenpaySign()){
                //当返回状态与业务结果都为0时才返回支付二维码，其它结果请查看接口文档
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){

                    $resFromHandler = array('code_img_url'=>$this->resHandler->getParameter('code_img_url'),
                                           'code_url'=>$this->resHandler->getParameter('code_url'),
                                           'code_status'=>$this->resHandler->getParameter('code_status'));

                    if ($serviceType == 'pay.alipay.webpay.intl') {
                        $resFromHandler['code_img_url'] = $this->resHandler->getParameter('pay_url');
                    }

                    return json_encode($resFromHandler);
