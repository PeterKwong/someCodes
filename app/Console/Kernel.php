...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        })->cron('*/8 * * * *')->between('00:01', '23:59')->runInBackground();
        ...
        $schedule->call(function () use(&$CronJob) {
            $this->diamondOncall($CronJob);
        })->cron('*/6 * * * *')->between('00:31', '01:30')->runInBackground();

        $schedule->call(function () use(&$CronJob) {
            $this->diamondOncall($CronJob);
        })->cron('*/6 * * * *')->between('02:00', '03:00')->runInBackground();

        $schedule->call(function () use(&$CronJob) {
            $this->diamondOncall($CronJob);
        })->cron('*/6 * * * *')->between('05:31', '06:30')->runInBackground();

        $schedule->call(function () use(&$CronJob) {
            $this->diamondOncall($CronJob);
        })->cron('*/6 * * * *')->between('08:00', '09:00')->runInBackground();
